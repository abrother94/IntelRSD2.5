...
#include "psme/rest/endpoints/acc_chassis/acc_chassis.hpp"
...
void fill_power_and_thermal_links(const agent_framework::model::Chassis& chassis, json::Json& r) {
    ...
    auto power = agent_framework::module::get_manager<agent_framework::model::Psu>()
        .get_keys(chassis.get_uuid());
    if (power.size() > 0) {
...
endpoint::Acc_Chassis::Acc_Chassis(const std::string& path) : EndpointBase(path) {}


endpoint::Acc_Chassis::~Acc_Chassis() {}


void endpoint::Acc_Chassis::get(const server::Request& request, server::Response& response) {
    ...
    //Todo
    //r[constants::Chassis::NETWORK_ADAPTERS][Common::ODATA_ID] = PathBuilder(request).append(constants::Chassis::NETWORK_ADAPTERS).build();
...
void endpoint::Acc_Chassis::patch(const server::Request& request, server::Response& response) {
